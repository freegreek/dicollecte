...
class dbaccessIE {
    ...
    */
    ...
    public function importThesaurus ($prj, $srcPathFile) {
        $hSrcFile = @fopen($srcPathFile, 'r');
        if (!$hSrcFile) {
            return array(FALSE, '_nofile');
        }
        $line = strtoupper(trim(fgets($hSrcFile)));
        if ($line != 'UTF-8') {
            return array(FALSE, '_noutf8');
        }
        $i = 0;
        $tmpPathFile = './export/' . $prj . '/_tmp_thes_' . time() . '.tabbed.txt';
        $hTmpFile = @fopen($tmpPathFile, 'w');
        if (!$hTmpFile) {
            return array(FALSE, '_nowrite');
        }
        while (!feof($hSrcFile)) {
            ...
            if ($line != '') {
                if (strpos('##', $line) !== FALSE) {
                    fclose($hSrcFile);
                    fclose($hTmpFile);
                    $this->db->errorReport($prj, 'The string “##” is not allowed in the thesaurus. Line ' . $i . ':' . PHP_EOL . $line);
                    return array(FALSE, '_nosharp');
                }
                if (!preg_match('`^[^|]+\|[1-9][0-9]*$`', $line)) {
                    fclose($hSrcFile);
                    fclose($hTmpFile);
                    $this->db->errorReport($prj, 'The line ' . $i+1 . ' does not describe a new entry' . PHP_EOL . 'Line: ' . $line);
                    return array(FALSE, '_nonewentrythes');
                }
                
                list($word, $nbclass) = explode('|', $line);
                $nbcl = (int) $nbclass;
                $syn = '';
                for ($j = 0;  $j < $nbcl;  $j++) {
                    $line = trim(fgets($hSrcFile));
                    ...
                    if (strpos('##', $line) !== FALSE) {
                        fclose($hSrcFile);
                        fclose($hTmpFile);
                        $this->db->errorReport($prj, 'The string “##” is not allowed in the thesaurus. Line ' . $i);
                        return array(FALSE, '_nosharp');
                    }
                    $syn .= ($j == 0) ? $line : '##' . $line;
                }
                fwrite($hTmpFile, $word . "\t" . $nbclass . "\t" . $syn . PHP_EOL);
            }
            $i++;
        }
        fclose($hSrcFile);
        ...
        // copy the temp file in the table
        $qTableName = 'dicl_'.$prj.'_thes (word, nbclass, syn)';
        list($ok, $msg) = $this->copyFileToTable($prj, $tmpPathFile, $qTableName, 'thesaurus');
        if (!$ok) {
            return array(FALSE, $msg);
        }
        unlink($tmpPathFile);
    ...
    public function exportThesaurus ($prj, $name_addon) {
        // thesaurus
        try {
            $oQ = $this->db->connx->query('SELECT * FROM dicl_'.$prj.'_thes ORDER BY word');
        }
        catch (PDOException $e) {
            $this->db->dbErrorReport($prj, $e, 'export thesaurus');
            return array(FALSE, '_dberror');
        }
        $fileThes = fopen('./export/'.$prj.'/'.$name_addon.'.dat', 'w');
        if (!$fileThes) {
            return array(FALSE, '_nowrite');
        }
        $encodingline = 'UTF-8' . PHP_EOL;
        fwrite($fileThes, $encodingline);
        $offset = strlen($encodingline);
        $idxTable = array();
        while ($data = $oQ->fetch(PDO::FETCH_ASSOC)) {
            $idxTable[$data['word']] = $offset;
            $line = $data['word'] . '|' . $data['nbclass'] . PHP_EOL;
            $offset = $offset + strlen($line);
            fwrite($fileThes, $line);
            $line = str_replace('##', PHP_EOL, $data['syn']) . PHP_EOL;
            $offset = $offset + strlen($line);
            fwrite($fileThes, $line);
        }
        fclose($fileThes);
        // index
        ksort($idxTable);
        $fileIdx = fopen('./export/'.$prj.'/'.$name_addon.'.idx', 'w');
        if (!$fileIdx) {
            return array(FALSE, '_nowrite');
        }
        fwrite($fileIdx, $encodingline);
        fwrite($fileIdx, count($idxTable) . PHP_EOL);
        foreach ($idxTable as $word => $offset) {
            fwrite($fileIdx, $word . '|' . $offset . PHP_EOL);
        }
        fclose($fileIdx);
        return array(TRUE, 'ok');
    }

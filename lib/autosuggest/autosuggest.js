...
/* 
	This library has been modified by Olivier R.
	Look for // ## new
*/
...
function autosuggest(id, array, url, onSelect)
{
	...
	this.response_time = 10;	// time, in milliseconds, between the last char typed and the actual query
	this.entry_limit   = 12;	// number of entries autocomplete will show at a time
    ...
    // ## new
	this.display_all     = (id == "sem" ? false : true);    // always display the full list
    this.insert_values   = true;    // insert values instead of keywords in the text field
	this.display_values  = true;    // display also values in the suggestion list
    // ##
...
autosuggest.prototype = {
	...
	build: function()
	{
		...
		str[cn++] = "<table cellspacing=\"0px\" cellpadding=\"0px\" id=\"suggest_table2_";
		...
		function addSuggestion(index, _first)
		{
			...
			var cell = row.insertCell(0);
			// ## new
			if (that.display_values) {
				if (that.values[index] != "") {
					cell.innerHTML = '<b class="values">' + that.values[index] + "</b> = " + that.keywords[index];
				}
				else {
					cell.innerHTML = '<b class="title">' + that.keywords[index] + "</b>";
				}
			}
			else {
			...
			//if(that.use_mouse)
			// ## new
			if((that.use_mouse && !that.display_values) || (that.values[index] != "" && that.display_values) )
			// ##
			{
